...
(org-export-define-derived-backend 'jekyll 'md
  ...
  :options-alist '((:layout "LAYOUT" nil jekyll-default-layout t)
                   ...
                   (:redirect-to "REDIRECT_TO" nil nil t)
...
(defun org-jekyll-template (contents info)
  (let (
        ...
        (toc (org-jekyll--format "toc: %s\n" (org-export-data (plist-get info :toc) info)))
        (redirect-from (org-jekyll--format "redirect_from: %s\n" (org-export-data (plist-get info :redirect-from) info)))
        (redirect-to (org-jekyll--format "redirect_to: %s\n" (org-export-data (plist-get info :redirect-to) info)))
    ...
    (concat "---\n" layout title date image published tags category categories toc redirect-from redirect-to "---\n\n" contents)))
